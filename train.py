...
class ImageSequence(keras.utils.Sequence):
    def __init__(self, directory_name, batch_size, captcha_length, captcha_symbols, captcha_width, captcha_height):
        ...
        #we need to chang backslash and colon back
        file_list_new = []
        for file in file_list:
            file = file.replace('BACKSLASH',"\\")
            file = file.replace('COLON',':')
            file_list_new.append(file)
        self.files = dict(zip(map(lambda x: x.split('.')[0], file_list_new), file_list))
        self.used_files = []
